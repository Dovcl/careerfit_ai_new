...
## 서버 실행
 - backend directory 에서 source venv/bin/activate
 - uvicorn main:app --reload --port 8080

 - frontend directory 에서 npm run dev
...
# 실행 중인 컨테이너 목록 확인
 - docker ps
# 컨테이너 종료
 - docker stop [CONTAINER ID 앞 3자리]
...
## 가장 안전한 해결 방법
 1. git switch -c backup-branch-rollback # 먼저 백업 브랜치를 만들기
 2. git switch main # 원래 브랜치로 돌아가기
 3. git checkout 커밋해시 # 방법 A : 이전 커밋 확인만
 4. git reset -hard 커밋해시 # 방법 B : 이전 커밋으로 복원
 5. git revert 커밋해시 # 방법 C : 이미 푸쉬한 커밋 취소
