...
이미 `careerfit.db`가 있을 때 8~9단계만 실행:

```bash
cd backend
python3 -c "
import pandas as pd
from data.preprocess import convert_to_rag_documents, save_rag_documents, DB_PATH, RAG_JSON
df = pd.read_sql('SELECT * FROM jobs', f'sqlite:///{DB_PATH}')
rag_docs = convert_to_rag_documents(df)
...
```
[jobs.csv / competitions.csv]
        │
        ▼ preprocess.py
[careerfit.db (SQLite)]  ← 15건 저장 ✅
[rag_documents.json]     ← RAG 문서 ✅
        │
        ▼ (연결 예정)
[FastAPI]
  ├── GET  /health
  ├── GET  /jobs          ← MOCK_JOBS (DB 미연동)
  └── POST /analyze
        │
        ▼
[llm_service.py]
  ├── MOCK_MODE=true  → 목업
  └── MOCK_MODE=false → Gemini 2.5 Flash-Lite
        ↑
  context_docs=[]  ← RAG 미연동
```
...
[SQLite] + [ChromaDB 벡터 DB]
        ▼ RAG 검색
[POST /analyze] → Gemini + 검색 결과 → 맞춤 답변 + sources
[GET /jobs]     → SQLite에서 실제 공고 조회
...
#### 남은 작업
- [ ] `competitions.csv` 전처리 및 DB/RAG 추가
- [ ] ChromaDB 벡터 임베딩 및 RAG 검색 파이프라인
- [ ] `GET /jobs` API → SQLite `careerfit.db` 연동
- [ ] `POST /analyze` → ChromaDB 검색 결과를 `context_docs`에 전달

---

### ⬜ 4일차: RAG 기반 서비스 + React UI (예정)

- [ ] `analyze` API에 ChromaDB RAG 컨텍스트 연동
- [ ] React + Vite 프론트엔드 UI
- [ ] FastAPI ↔ React 연동
...
### `IndentationError` (preprocess.py)

- 함수 정의는 파일 **위쪽**
- `if __name__ == "__main__":` 실행 코드는 **맨 아래**
- `def` 앞 불필요한 공백 금지
...
- Repository: https://github.com/Dovcl/careerfit_ai_new
- `.env`, `venv/`, `__pycache__/`, `*.db`는 `.gitignore`에 포함 권장
